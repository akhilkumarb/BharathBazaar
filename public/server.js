...
app.post("/checkid", async (req, res) => {
    try {
        const { userid, register } = req.body;
        const producer = await producers.findOne({ userid: userid });
        const assignedid = producer.assignedid;
        if (assignedid == register) {
            await users.updateOne({ _id: userid }, {
                $set: {
                    Itemproduced: 1
                }
            })
            res.redirect(`/user/addproduct?userid=${userid}`);
        }
        else {
